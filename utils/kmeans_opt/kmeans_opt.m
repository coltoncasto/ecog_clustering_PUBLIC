function [IDX,C,SUMD,D,K,d,Var,PC]=kmeans_opt(X,varargin)
...
for c=1:ToTest %for each sample
    [~,~,dist]=kmeans(X,c,'emptyaction','drop','Distance',Distance); %compute the sum of intra-cluster distances
    tmp=sum(dist); %best so far
    
    for cc=2:Repeats %repeat the algo
        [~,~,dist]=kmeans(X,c,'emptyaction','drop','Distance',Distance);
...
[IDX,C,SUMD,D]=kmeans(X,K,'Distance',Distance); %now rerun one last time with the optimal number of clusters
